...
{
    /**
     * Pointer to the structure with SAADC peripheral instance registers.
     */
 ...
 * SDK it appears that a read of the EVENTS_XXX register is required to
 * complete the operation of clearing the event.
 */
...
void saadc_deinit(void)
{
    ...
    /// @todo This is not properly thought out.
                            ...
                            saadc_event_handler_t saadc_handler)
{
    ...
    struct saadc_conversion_channel_time_t const channel_conversion =
        saadc_conversion_channel_time();
    ...
    saadc_registers->INTENSET =
        ...
        SAADC_INTEN_END_Msk         |
//      SAADC_INTEN_DONE_Msk        |       // for debug only.
//      SAADC_INTEN_RESULTDONE_Msk  |       // for debug only.
...
void saadc_conversion_stop(void)
{
    ...
    /// @todo Clearing and disabling interrupts is not the correct action
    /// if the client is depending on the STOP event.
    NVIC_DisableIRQ(saadc_instance_0.irq_type);
    saadc_registers->INTENCLR = interrupts_clear_all;

    saadc_registers->INTENSET   = SAADC_INTEN_STOPPED_Msk;
    saadc_registers->ENABLE     = 0u;
    saadc_registers->TASKS_STOP = 1u;   // Note: This generates a STOP event.
}

struct saadc_conversion_channel_time_t saadc_conversion_channel_time(void)
{
    saadc_conversion_channel_time_t channel_conversion = {
        .time_usec     = 0u,
        .channel_count = 0u
    };
...
static void irq_handler_saadc(struct saadc_control_block_t* saadc_control)
{
    ...
    uint8_t const event_value_ignore = UINT8_MAX;
    logger& logger = logger::instance();

    if (saadc_registers->EVENTS_STARTED)
    {
        logger.debug("IRQ: EVENTS_STARTED");
        saadc_registers->TASKS_SAMPLE = 1u;
        saadc_clear_event_register(&saadc_registers->EVENTS_STARTED);
